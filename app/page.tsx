 ...
 *  Wider than HIDDEN_COLS, which keeps the ones the grid still displays. */
 ...
 */
function resolveFormType(f: LoadedFile): FormType {
  const tagged = f.formType ?? detectFormType(f.headers);
  if (f.formTypeSource === 'manual' || tagged !== 'merch') return tagged;

  const questions = f.headers.filter(h => !isHiddenCol(h) && !METADATA_COLS.has(h.toLowerCase().trim()));
...
export default function Dashboard() {
  ...
  // formType is already resolved for every file in loadedFiles (see
  // loadChannelsData), so this reads it directly rather than re-deriving it.
  const formFilteredFiles = useMemo(
    () => loadedFiles.filter(f => f.formType === selFormType),
  ...
  const mergedData = useMemo(() => {
    ...
    const rows: VisitRow[] = formFilteredFiles.flatMap(f =>
      f.rows.map(r => ({ ...r, _source: f.name, _formType: f.formType } as VisitRow))
    );
